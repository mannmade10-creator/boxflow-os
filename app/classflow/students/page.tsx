...
const STATS = [
  ...
  { label: 'Active',            value: STUDENTS.filter(s => s.status === 'Active').length,          icon: '✅', color: '#22c55e' },
  { label: 'At Risk',           value: STUDENTS.filter(s => s.status === 'At Risk').length,         icon: '⚠️', color: '#f43f5e' },
  { label: 'Avg Score',         value: Math.round(STUDENTS.reduce((a,s) => a + s.avgScore, 0) / STUDENTS.length) + '%', icon: '📊', color: '#a78bfa' },
]
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.txt, fontFamily: C.D }}>
      <style>{`
        ...
        .modal-act{background:transparent;border:0.5px solid rgba(59,130,246,.25);border-radius:9px;color:#f1f5f9;font-size:12px;font-family:'Outfit',sans-serif;font-weight:500;padding:10px 14px;cursor:pointer;text-align:left;transition:all .15s}
      ...
      <main style={{ padding: '28px 24px', maxWidth: 1100, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button style={{ padding: '10px 18px', background: C.blue, border: 'none', borderRadius: 10, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: C.D }}>
            + Invite Student
          </button>
        </div>
        ...
        <div style={{ background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 14, overflow: 'hidden' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', borderBottom: `0.5px solid ${C.border}`, gap: 12, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['All', 'Active', 'At Risk'].map(f => (
                <button key={f} onClick={() => setFilter(f)} style={{ padding: '6px 14px', borderRadius: 8, border: `0.5px solid ${filter === f ? C.blue : C.border}`, background: filter === f ? 'rgba(59,130,246,0.1)' : 'transparent', color: filter === f ? C.blue : C.dim, fontSize: 12, cursor: 'pointer', fontFamily: C.D }}>
                  {f}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', gap: 12, padding: '8px 20px', borderBottom: `0.5px solid ${C.border}` }}>
            {['Student', 'Grade', 'Lessons', 'Avg Score', 'Last Seen', 'Status', ''].map((h, i) => (
              <span key={h + i} style={{ ...(h === 'Student' ? { flex: 1, minWidth: 0 } : h === '' ? { width: 80, flexShrink: 0 } : { width: h === 'Lessons' || h === 'Grade' ? 70 : h === 'Last Seen' ? 100 : 80, flexShrink: 0 }), fontSize: 9, color: C.dim, letterSpacing: 1.8, fontFamily: C.M, textTransform: 'uppercase' }}>{h}</span>
            ))}
          </div>
        ...
        <div onClick={() => setModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(4,5,10,.88)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, backdropFilter: 'blur(5px)' }}>
          <div onClick={(e: any) => e.stopPropagation()} style={{ background: C.panel, border: `0.5px solid rgba(59,130,246,.3)`, borderRadius: 18, padding: '28px 30px', maxWidth: 420, width: '92%' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
              <div style={{ width: 48, height: 48, borderRadius: '50%', background: `${C.blue}20`, border: `0.5px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 700, color: C.blue }}>{modal.name.charAt(0)}</div>
              <div>
                <h2 style={{ fontSize: 17, fontWeight: 800, color: C.txt, margin: 0 }}>{modal.name}</h2>
                <p style={{ fontSize: 12, color: C.dim, margin: 0, fontFamily: C.M }}>{modal.grade} · {modal.lessons} lessons · Last seen {modal.lastSeen}</p>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
              {[['Avg Score', `${modal.avgScore}%`], ['Status', modal.status], ['Lessons', modal.lessons], ['Grade', modal.grade]].map(([l, v]) => (
                <div key={l} style={{ background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 10, padding: '12px 14px' }}>
                  <div style={{ fontSize: 10, color: C.dim, fontFamily: C.M, letterSpacing: 1.5, marginBottom: 4 }}>{l.toUpperCase()}</div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {modal.courses.map((c: string) => (
                  <div key={c} style={{ fontSize: 12, color: C.txt, background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 8, padding: '8px 12px' }}>{c}</div>
                ))}
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
              {['📊 View Progress', '📧 Send Message', '📚 Assign Lesson', '⚠️ Flag for Review'].map(a => (
