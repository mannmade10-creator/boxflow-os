    ...
    <div style={{ minHeight: '100vh', background: '#0c1324', color: '#dce1fb', fontFamily: 'Inter, Arial, sans-serif' }}>
      <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', background: 'rgba(12,19,36,0.96)', backdropFilter: 'blur(24px)', borderBottom: '1px solid rgba(180,197,255,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 13, color: 'rgba(195,198,215,0.5)', fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase' }}>FLEET_COMMAND</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.15)', borderRadius: 3, padding: '3px 10px' }}>
            ...
            <span style={{ color: '#22c55e', fontSize: 13, fontWeight: 700, letterSpacing: 2 }}>GEO SYNC ACTIVE</span>
          </div>
          <a href="/dashboard" style={{ fontSize: 13, color: 'rgba(180,197,255,0.5)', textDecoration: 'none', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' }}>BACK</a>
        </div>
      </header>
      ...
      {!fullscreen && (
        <aside style={{ position: 'fixed', left: 0, top: 56, bottom: 0, width: 200, background: '#151b2d', zIndex: 50, display: 'flex', flexDirection: 'column' }} className="kinetic-sidebar">
          <div style={{ padding: '20px 16px 12px' }}>
            <div style={{ fontSize: 12, color: 'rgba(195,198,215,0.35)', fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', marginBottom: 4 }}>Navigation</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
            {!fullscreen && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', background: '#0c1324', flexShrink: 0 }}>
                <div>
                  <div style={{ fontSize: 12, color: '#b4c5ff', fontWeight: 800, letterSpacing: 3, textTransform: 'uppercase', marginBottom: 2 }}>System Status: Optimal</div>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  {[
                    ...
                    <div key={stat.label} style={{ background: '#151b2d', padding: '6px 12px', borderRadius: 3, textAlign: 'right' }}>
                      <div style={{ fontSize: 11, color: 'rgba(195,198,215,0.4)', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 2 }}>{stat.label}</div>
            ...
            <div style={{ position: 'relative', flex: 1, minHeight: 0, overflow: 'hidden', background: '#191f31' }}>
              ...
              <div style={{ position: 'absolute', bottom: 16, left: 16, zIndex: 10, pointerEvents: 'none', background: 'rgba(21,27,45,0.82)', backdropFilter: 'blur(20px)', padding: '10px 14px', borderRadius: 3, border: '1px solid rgba(180,197,255,0.08)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 800, color: '#fff', letterSpacing: 3, textTransform: 'uppercase' }}>Live Telemetry</span>
                </div>
                <div style={{ fontSize: 13, color: '#7bd0ff', lineHeight: 2 }}>GEO SYNC: ACTIVE<br/>TRUCKS: 3 ACTIVE<br/>REGION: OKC METRO</div>
              </div>
              <div style={{ position: 'absolute', bottom: 16, right: 16, zIndex: 10 }}>
                <button onClick={() => setFullscreen(!fullscreen)} style={{ background: 'rgba(21,27,45,0.85)', backdropFilter: 'blur(20px)', border: '1px solid rgba(180,197,255,0.12)', borderRadius: 3, padding: '8px 16px', color: '#b4c5ff', fontWeight: 800, fontSize: 13, cursor: 'pointer', letterSpacing: 2, textTransform: 'uppercase' }}>
                ...
                {[
                  ...
                  <div key={m.label} style={{ background: '#151b2d', padding: '12px 16px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                      <span style={{ fontSize: 11, color: 'rgba(195,198,215,0.35)', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' }}>{m.label}</span>
          ...
          {!fullscreen && (
            <div style={{ background: '#151b2d', borderLeft: '1px solid rgba(180,197,255,0.04)', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
              <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(180,197,255,0.04)', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 3, textTransform: 'uppercase', color: '#dce1fb' }}>Active Personnel</span>
                <span style={{ fontSize: 12, color: '#b4c5ff', fontWeight: 700 }}>{drivers.length} UNITS</span>
              </div>
                  ...
                  <div key={d.id} onClick={() => setSelected(selected === d.id ? null : d.id)} style={{ padding: '14px 16px', cursor: 'pointer', background: selected === d.id ? 'rgba(37,99,235,0.08)' : 'transparent', borderBottom: '1px solid rgba(180,197,255,0.03)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 5px', borderRadius: 2, background: d.bg, color: d.color }}>{d.status}</span>
                        </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 6 }}>
                      <div style={{ background: 'rgba(12,19,36,0.5)', borderRadius: 2, padding: '4px 8px' }}>
                        <div style={{ fontSize: 11, color: 'rgba(195,198,215,0.35)', marginBottom: 1 }}>ETA</div>
                        <div style={{ fontSize: 10, fontWeight: 700, color: '#dce1fb' }}>{d.eta}</div>
                      </div>
                      <div style={{ background: 'rgba(12,19,36,0.5)', borderRadius: 2, padding: '4px 8px' }}>
                        <div style={{ fontSize: 11, color: 'rgba(195,198,215,0.35)', marginBottom: 1 }}>FUEL</div>
              ...
              <div style={{ borderTop: '1px solid rgba(180,197,255,0.04)', flexShrink: 0 }}>
                <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span>🤖</span>
                  <span style={{ fontSize: 12, fontWeight: 800, letterSpacing: 3, textTransform: 'uppercase', color: '#dce1fb' }}>AI Optimization</span>
                </div>
                {[
                  ...
                  <div key={a.title} style={{ padding: '10px 16px', borderTop: '1px solid rgba(180,197,255,0.03)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: 12, color: a.color, fontWeight: 700 }}>{a.title}</span>
                      <span style={{ fontSize: 11, color: 'rgba(195,198,215,0.4)', fontWeight: 700 }}>{a.badge}</span>
                    </div>
                    <p style={{ fontSize: 13, color: 'rgba(195,198,215,0.5)', lineHeight: 1.5, margin: '0 0 6px' }}>{a.desc}</p>
                    <button style={{ width: '100%', padding: '5px', background: 'rgba(180,197,255,0.05)', border: '1px solid rgba(180,197,255,0.1)', borderRadius: 2, color: '#b4c5ff', fontSize: 11, fontWeight: 800, letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer' }}>APPLY CHANGE</button>
                  </div>
                ))}
              </div>
            </div>
