  ...
  return (
    <main style={{ minHeight: '100vh', background: '#050d1a', color: '#e2e8f0', fontFamily: 'Inter,Arial,sans-serif' }}>
      <style>{`
        ...
        .staff-pin:hover{transform:scale(1.15);transition:transform 0.15s}
      `}</style>
      ...
      <div style={{ maxWidth: 1300, margin: '0 auto', padding: 24 }}>
        ...
        {busArrived && (
          <div style={{ background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.4)', borderRadius: 12, padding: '14px 20px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: 8, background: 'rgba(245,158,11,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: '#f59e0b', flexShrink: 0 }}>BUS</div>
            <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 300px', gap: 20, alignItems: 'start' }}>
          ...
          {/* MAP */}
          <div style={{ background: '#0a1628', border: '1px solid rgba(79,142,247,0.2)', borderRadius: 16, overflow: 'hidden', position: 'relative' as const, minHeight: 480 }}>
            {/* Grid lines */}
            <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(79,142,247,0.06) 1px,transparent 1px),linear-gradient(90deg,rgba(79,142,247,0.06) 1px,transparent 1px)', backgroundSize: '40px 40px' }} />

            {/* Property boundary */}
            <div style={{ position: 'absolute', top: '15%', left: '10%', right: '10%', bottom: '10%', border: '2px dashed rgba(34,197,94,0.3)', borderRadius: 8 }}>
            ...
            {/* Buildings */}
            {[
              { w: '18%', h: '12%', t: '20%', l: '15%', label: 'Bldg A' },
              { w: '18%', h: '12%', t: '20%', l: '40%', label: 'Bldg B' },
              { w: '18%', h: '12%', t: '20%', l: '65%', label: 'Bldg C' },
              { w: '18%', h: '12%', t: '42%', l: '15%', label: 'Bldg D' },
              { w: '18%', h: '12%', t: '42%', l: '65%', label: 'Bldg E' },
              { w: '28%', h: '10%', t: '64%', l: '35%', label: 'Leasing Office' },
            ].map((b, i) => (
              <div key={i} style={{ position: 'absolute', top: b.t, left: b.l, width: b.w, height: b.h, background: 'rgba(79,142,247,0.08)', border: '1px solid rgba(79,142,247,0.25)', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 9, color: '#4f8ef7', fontWeight: 700 }}>{b.label}</span>
              </div>
            ))}

            {/* Roads */}
            <div style={{ position: 'absolute', top: '57%', left: '10%', right: '10%', height: 6, background: 'rgba(100,116,139,0.2)', borderRadius: 3 }} />
            <div style={{ position: 'absolute', top: '15%', bottom: '10%', left: '35%', width: 6, background: 'rgba(100,116,139,0.2)', borderRadius: 3 }} />
            <div style={{ position: 'absolute', top: '15%', bottom: '10%', left: '60%', width: 6, background: 'rgba(100,116,139,0.2)', borderRadius: 3 }} />

            {/* Staff pins */}
            {staffData.map(s => (
              <div key={s.id} className="staff-pin"
                onClick={() => setSelectedStaff(selectedStaff?.id === s.id ? null : s)}
                style={{ position: 'absolute', top: s.top, left: s.left, transform: 'translate(-50%,-50%)', cursor: 'pointer', zIndex: 10 }}>
                <div style={{ position: 'relative' }}>
                  <div style={{ width: 34, height: 34, borderRadius: '50%', background: s.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#fff', border: '2px solid #fff', boxShadow: `0 0 12px ${s.color}` }}>
                    {s.name.split(' ').map(n => n[0]).join('')}
                  </div>
                  <div style={{ position: 'absolute', inset: -4, borderRadius: '50%', border: `2px solid ${s.color}`, animation: 'ping 2s ease-out infinite', opacity: 0 }} />
                </div>
                <div style={{ fontSize: 9, color: '#fff', textAlign: 'center' as const, marginTop: 3, textShadow: '0 1px 4px rgba(0,0,0,0.9)', fontWeight: 700 }}>{s.name.split(' ')[0]}</div>
              </div>
            ))}

            {/* Property label */}
            <div style={{ position: 'absolute', top: 10, left: 10, background: 'rgba(2,8,18,0.85)', padding: '6px 10px', borderRadius: 6 }}>
            ...
            {/* Legend */}
            <div style={{ position: 'absolute', bottom: 10, left: 10, background: 'rgba(2,8,18,0.85)', padding: '8px 12px', borderRadius: 8 }}>
            ...
            {/* Staff count */}
            <div style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(2,8,18,0.85)', padding: '6px 10px', borderRadius: 6, fontSize: 10, color: '#22c55e', fontWeight: 700 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 12 }}>
            <div style={{ background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(79,142,247,0.12)', borderRadius: 14, padding: 16 }}>
              ...
              {staffData.map(s => (
                <div key={s.id} onClick={() => setSelectedStaff(selectedStaff?.id === s.id ? null : s)}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 6px', borderBottom: '1px solid rgba(79,142,247,0.07)', cursor: 'pointer', borderRadius: 6, background: selectedStaff?.id === s.id ? `${s.color}11` : 'transparent' }}>
            ...
            {selectedStaff && (
              <div style={{ background: 'rgba(15,23,42,0.9)', border: `1px solid ${selectedStaff.color}44`, borderRadius: 14, padding: 16 }}>
                ...
                <div style={{ fontSize: 11, color: '#22c55e', fontWeight: 700 }}>On Property</div>
              </div>
            )}
            ...
            <div style={{ background: 'rgba(15,23,42,0.9)', border: '1px solid rgba(79,142,247,0.12)', borderRadius: 14, padding: 16 }}>
              ...
              <button onClick={activateBus} disabled={busOnRoute}
                style={{ width: '100%', padding: '10px', background: busOnRoute ? 'rgba(245,158,11,0.3)' : '#f59e0b', border: 'none', borderRadius: 8, color: '#000', fontWeight: 800, fontSize: 12, cursor: busOnRoute ? 'not-allowed' : 'pointer', fontFamily: 'Inter,Arial,sans-serif' }}>
