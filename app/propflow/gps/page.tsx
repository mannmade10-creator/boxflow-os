...
export default function GPSPage() {
  ...
  if (!mounted) return (
    <div style={{ minHeight: '100vh', background: '#050d1a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ color: '#475569', fontSize: 13, fontFamily: 'system-ui' }}>Loading GPS...</div>
    </div>
  )
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#050d1a', color: '#e2e8f0', fontFamily: 'system-ui,Arial,sans-serif' }}>
      <style>{`
        ...
        .staff-pin{cursor:pointer;transition:transform 0.15s}
        .staff-pin:hover{transform:scale(1.15) translate(-43%,-43%)}
      `}</style>
      ...
      <div style={{ maxWidth: 1300, margin: '0 auto', padding: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <span suppressHydrationWarning style={{ fontFamily: 'monospace', color: '#4f8ef7', fontSize: 12 }}>
              {timeStr}
            </span>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 300px', gap: 20, alignItems: 'start' }}>
          ...
          <div style={{ background: '#0a1628', border: '1px solid rgba(79,142,247,0.2)', borderRadius: 16, overflow: 'hidden', position: 'relative', minHeight: 480 }}>
            ...
            {/* Property boundary */}
            <div style={{ position: 'absolute', top: '12%', left: '8%', right: '8%', bottom: '8%', border: '2px dashed rgba(34,197,94,0.25)', borderRadius: 8, pointerEvents: 'none' }}>
              <div style={{ position: 'absolute', top: -10, left: 12, background: '#0a1628', padding: '0 6px', fontSize: 9, color: '#22c55e', fontWeight: 700, letterSpacing: 1 }}>PENN STATION PROPERTY BOUNDARY</div>
            </div>
            ...
            {staffData.map(s => (
              <div key={s.id} className="staff-pin"
                ...
                <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ position: 'absolute', width: 34, height: 34, borderRadius: '50%', border: `2px solid ${s.color}`, top: 0, left: 0, animation: 'ping2 2s ease-out infinite', opacity: 0 }} />
                  <div style={{ width: 34, height: 34, borderRadius: '50%', background: s.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: '#fff', border: '2px solid rgba(255,255,255,0.8)', boxShadow: `0 0 12px ${s.color}80`, position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ position: 'absolute', top: 10, left: 10, background: 'rgba(2,8,18,0.88)', padding: '6px 10px', borderRadius: 6, pointerEvents: 'none' }}>
              ...
              <div style={{ fontSize: 9, color: '#475569' }}>1920 Heritage Park Dr, OKC 73120</div>
            </div>
