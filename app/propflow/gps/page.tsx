...
const LEGEND = [
  ...
  { label: 'Make-Ready',  color: '#22c55e' },
  { label: 'Security',    color: '#ef4444' },
  { label: 'Admin',       color: '#a855f7' },
]
...
export default function GPSPage() {
  ...
  const [mapSize, setMapSize]       = useState({ w: 900, h: 540 })
  ...
  const mapUrl = token
    ? `https://api.mapbox.com/styles/v1/mapbox/satellite-streets-v12/static/-97.5572,35.5938,16,0/${mapSize.w}x${mapSize.h}?access_token=${token}`
    : ''
  ...
  return (
    <main style={{ minHeight:'100vh', background:'#050d1a', color:'#e2e8f0', fontFamily:'system-ui,Arial,sans-serif' }}>
      <style>{`
        ...
        .bus-pin{transition:top 1s ease,left 1s ease}
        ...
        .propflow-nav-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;white-space:nowrap}
        .propflow-nav-wrap::-webkit-scrollbar{display:none}
        .propflow-nav-wrap a{display:inline-block}
      ...
      <header style={{ background:'#070f1f', borderBottom:'1px solid rgba(99,132,255,0.15)', padding:'0 16px', height:60, display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}>
        ...
        <div className="propflow-nav-wrap" style={{ flex:1, marginLeft:12 }}>
          <nav style={{ display:'inline-flex', gap:2 }}>
            {NAV.map(item => (
              <a key={item} href={item==='Dashboard'?'/propflow/dashboard':`/propflow/${item.toLowerCase()}`}
                style={{ padding:'6px 10px', fontSize:11, fontWeight:700, color:item==='GPS'?'#4f8ef7':'#475569', borderRadius:7, textDecoration:'none', background:item==='GPS'?'rgba(79,142,247,0.1)':'transparent', whiteSpace:'nowrap' }}>
                {item}
              </a>
            ))}
          </nav>
      ...
      <div style={{ maxWidth:1300, margin:'0 auto', padding:'16px 16px 24px' }}>

        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, flexWrap:'wrap', gap:10 }}>
          <div>
            <h1 style={{ fontSize:22, fontWeight:800, color:'#fff', marginBottom:4 }}>GPS Live Tracker</h1>
            <p style={{ fontSize:12, color:'#475569' }}>Penn Station — 1920 Heritage Park Dr, OKC 73120</p>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
            <span style={{ fontSize:10, color:'#22c55e', fontWeight:700, background:'rgba(34,197,94,0.1)', padding:'4px 10px', borderRadius:20, border:'1px solid rgba(34,197,94,0.2)', whiteSpace:'nowrap' }}>GEO SYNC ACTIVE</span>
            <span style={{ fontFamily:'monospace', color:'#4f8ef7', fontSize:11 }}>{timeStr}</span>
            <button onClick={() => setShowPanel(p => !p)}
              style={{ padding:'6px 12px', background:'rgba(79,142,247,0.1)', border:'1px solid rgba(79,142,247,0.3)', borderRadius:8, color:'#4f8ef7', fontSize:11, fontWeight:700, cursor:'pointer', fontFamily:'system-ui', whiteSpace:'nowrap' }}>
        ...
        {busArrived && (
          <div style={{ background:'rgba(245,158,11,0.15)', border:'1px solid rgba(245,158,11,0.4)', borderRadius:12, padding:'12px 16px', marginBottom:16, display:'flex', alignItems:'center', gap:12 }}>
        ...
        {busOnRoute && (
          <div style={{ background:'rgba(79,142,247,0.1)', border:'1px solid rgba(79,142,247,0.3)', borderRadius:12, padding:'12px 16px', marginBottom:16 }}>
            <div style={{ fontSize:13, fontWeight:700, color:'#4f8ef7' }}>School bus is en route — ETA 5 minutes...</div>
          </div>
        )}

        <div className={`gps-grid${showPanel ? '' : ' full'}`}>

          <div className="gps-map">
            {token && !mapError && (
              <img src={mapUrl} alt="Property Map" onError={() => setMapError(true)}
                style={{ width:'100%', height:'100%', display:'block', objectFit:'cover', minHeight:300 }} />
            )}
            {(!token || mapError) && (
              <div style={{ width:'100%', minHeight:360, background:'#0a1628', backgroundImage:'linear-gradient(rgba(79,142,247,0.05) 1px,transparent 1px),linear-gradient(90deg,rgba(79,142,247,0.05) 1px,transparent 1px)', backgroundSize:'40px 40px', position:'relative' }}>
                <div style={{ position:'absolute', top:'12%', left:'8%', right:'8%', bottom:'8%', border:'2px dashed rgba(34,197,94,0.22)', borderRadius:8 }}>
                  <div style={{ position:'absolute', top:-10, left:12, background:'#0a1628', padding:'0 6px', fontSize:9, color:'#22c55e', fontWeight:700 }}>PENN STATION PROPERTY</div>
                </div>
                {[
                  ...
                  <div key={i} style={{ position:'absolute',top:b.t,left:b.l,width:b.w,height:b.h,background:'rgba(79,142,247,0.07)',border:'1px solid rgba(79,142,247,0.22)',borderRadius:4,display:'flex',alignItems:'center',justifyContent:'center' }}>
                    <span style={{ fontSize:8,color:'#4f8ef7',fontWeight:700,textAlign:'center' }}>{b.label}</span>
                  </div>
                ))}
                <div style={{ position:'absolute',top:'53%',left:'8%',right:'8%',height:7,background:'rgba(100,116,139,0.18)',borderRadius:4 }} />
                <div style={{ position:'absolute',top:'12%',bottom:'8%',left:'33%',width:7,background:'rgba(100,116,139,0.18)',borderRadius:4 }} />
                <div style={{ position:'absolute',top:'12%',bottom:'8%',left:'59%',width:7,background:'rgba(100,116,139,0.18)',borderRadius:4 }} />
              </div>
            )}
            ...
            {STAFF.map(s => (
              <div key={s.id} className="spin" onClick={() => setSelected(selected?.id===s.id?null:s)}
                ...
                <div style={{ display:'flex',flexDirection:'column',alignItems:'center' }}>
                  ...
                  <div style={{ fontSize:9,color:'#fff',marginTop:3,textShadow:'0 1px 6px rgba(0,0,0,1)',fontWeight:700,whiteSpace:'nowrap' }}>{s.name.split(' ')[0]}</div>
                </div>
              </div>
            ))}
              ...
              <div className="bus-pin"
                ...
                <div style={{ display:'flex',flexDirection:'column',alignItems:'center' }}>
                  <div style={{ position:'relative',width:42,height:42 }}>
                    ...
                    <div style={{ width:42,height:42,borderRadius:'50%',background:'#f59e0b',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:900,color:'#000',border:'2px solid rgba(255,255,255,0.9)',boxShadow:'0 0 20px rgba(245,158,11,0.8)',position:'relative',zIndex:1 }}>BUS</div>
                  </div>
            ...
            <div style={{ position:'absolute',top:10,right:10,background:'rgba(2,8,18,0.88)',padding:'6px 10px',borderRadius:6,fontSize:10,color:'#22c55e',fontWeight:700,zIndex:5 }}>
              {STAFF.length} ON PROPERTY
            </div>
            <div style={{ position:'absolute',bottom:10,left:10,background:'rgba(2,8,18,0.88)',padding:'8px 12px',borderRadius:8,zIndex:5 }}>
          ...
          {showPanel && (
            <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
              ...
              {selected && (
                <div style={{ background:'rgba(15,23,42,0.9)',border:`1px solid ${selected.color}44`,borderRadius:14,padding:16 }}>
                  <div style={{ display:'flex',justifyContent:'space-between',marginBottom:8 }}>
                    <div style={{ fontSize:13,fontWeight:700,color:selected.color }}>{selected.name}</div>
                    <button onClick={() => setSelected(null)} style={{ background:'none',border:'none',color:'#475569',cursor:'pointer',fontSize:18,lineHeight:1 }}>x</button>
                  </div>
                  <div style={{ fontSize:12,color:'#94a3b8',marginBottom:6 }}>{selected.role}</div>
                  <div style={{ fontSize:11,color:'#22c55e',fontWeight:700,marginBottom:10 }}>On Property</div>
                  <div style={{ display:'flex',gap:8 }}>
                    <button style={{ flex:1,padding:'7px',background:`${selected.color}18`,border:`1px solid ${selected.color}40`,borderRadius:8,color:selected.color,fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'system-ui' }}>Message</button>
