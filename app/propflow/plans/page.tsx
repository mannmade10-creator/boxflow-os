...
export default function PlansPage() {
  ...
  const [activePlan, setActive] = useState<string>('professional')
    ...
    <div style={{ minHeight: '100vh', background: '#050d1a', color: '#f0f6ff', fontFamily: 'system-ui,sans-serif', paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 20, marginBottom: 64 }}>
          ...
          {/* LANDLORD */}
          <div className="card-hover" style={{ background: '#070f1f', border: `2px solid ${activePlan === 'landlord' ? BLUE : 'rgba(79,142,247,0.15)'}`, borderRadius: 20, padding: 32, position: 'relative' }}>
            ...
            <a href="https://buy.stripe.com/landlord_monthly"
              style={{ display: 'block', textAlign: 'center', padding: '13px', background: 'rgba(79,142,247,0.12)', border: `1px solid ${BLUE}40`, borderRadius: 12, color: BLUE, fontWeight: 800, fontSize: 15, textDecoration: 'none' }}>
          ...
          <div className="card-hover" style={{ background: '#0a0a1a', border: `2px solid ${PURPLE}`, borderRadius: 20, padding: 32, position: 'relative', transform: 'scale(1.02)' }}>
            ...
            <a href="https://buy.stripe.com/professional_monthly"
        ...
        <div style={{ marginBottom: 64 }}>
          <div style={{ textAlign: 'center', marginBottom: 36 }}>
            ...
            <p style={{ fontSize: 14, color: '#64748b', maxWidth: 480, margin: '0 auto' }}>
              Start at $89/mo and add only the features you need. Each add-on unlocks instantly after payment.
            </p>
          </div>
            ...
            {LANDLORD_ADDONS.map(key => {
              ...
              return (
                <div key={key} className="card-hover" style={{ background: '#070f1f', border: '1px solid rgba(79,142,247,0.12)', borderRadius: 16, padding: '22px 22px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <a href={`https://buy.stripe.com/addon_${key}`}
          ...
          <div style={{ marginTop: 24, padding: '20px 24px', background: 'rgba(79,142,247,0.06)', border: '1px solid rgba(79,142,247,0.15)', borderRadius: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 14 }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: '#475569' }}>
                $89 base + ${LANDLORD_ADDONS.reduce((a, k) => a + ADDON_INFO[k].price, 0)} add-ons = ${89 + LANDLORD_ADDONS.reduce((a, k) => a + ADDON_INFO[k].price, 0)}/mo total
              </div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 11, color: GREEN, fontWeight: 700 }}>Save ${(249 - (89 + LANDLORD_ADDONS.reduce((a, k) => a + ADDON_INFO[k].price, 0)))}/mo vs Professional</div>
              <div style={{ fontSize: 11, color: '#475569', marginTop: 2 }}>
                {249 - (89 + LANDLORD_ADDONS.reduce((a, k) => a + ADDON_INFO[k].price, 0)) > 0
                  ? '→ Upgrade to Professional for fewer, lower price'
                  : '→ Landlord + all add-ons costs more — Professional is the better deal'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 64 }}>
          ...
          <div style={{ background: '#070f1f', border: '1px solid rgba(79,142,247,0.1)', borderRadius: 16, overflow: 'hidden' }}>
            ...
            {[
              { feature: 'Units',              landlord: 'Up to 10',    pro: 'Up to 100',   ent: 'Unlimited' },
              { feature: 'User Seats',         landlord: '1 user',      pro: '5 users',     ent: 'Unlimited' },
              { feature: 'Tenant Portal',      landlord: '✓',           pro: '✓',           ent: '✓' },
              { feature: 'Tenant Mobile App',  landlord: '✓',           pro: '✓',           ent: 'White-label' },
              { feature: 'Maintenance Tracking', landlord: '✓',         pro: '✓',           ent: '✓' },
              { feature: 'Billing Statements', landlord: '✓',           pro: '✓',           ent: '✓' },
              { feature: 'ACH Payments',       landlord: '+$19/mo',     pro: '✓ Included',  ent: '✓ Included' },
              { feature: 'Income Verification',landlord: '+$14/mo',     pro: '✓ Included',  ent: '✓ Included' },
              { feature: 'Accounting & P&L',   landlord: '+$24/mo',     pro: '✓ Included',  ent: '✓ Included' },
              { feature: 'Payroll & HR',        landlord: '+$39/mo',    pro: '✓ Included',  ent: '✓ Included' },
              { feature: 'Business Analytics', landlord: '+$19/mo',     pro: '✓ Included',  ent: '✓ Included' },
              { feature: 'GPS Bus Tracker',    landlord: '+$9/mo',      pro: '✓ Included',  ent: '✓ Included' },
              { feature: 'Community Board',    landlord: '+$9/mo',      pro: '✓ Included',  ent: '✓ Included' },
              { feature: 'QuickBooks Sync',    landlord: '—',           pro: 'Coming soon', ent: '✓ Included' },
              { feature: 'White-label App',    landlord: '—',           pro: '—',           ent: '✓ Included' },
              { feature: 'Support',            landlord: 'Email',       pro: 'Priority',    ent: 'Dedicated' },
            ].map((row, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 120px 120px 120px', padding: '12px 24px', borderBottom: i < 15 ? '1px solid rgba(255,255,255,0.04)' : 'none', alignItems: 'center' }}>
                ...
                <div style={{ textAlign: 'center', fontSize: 12, color: row.landlord.startsWith('+') ? '#4f8ef7' : row.landlord === '—' ? '#1e3a5f' : GREEN, fontWeight: row.landlord === '✓' || row.landlord.startsWith('+') ? 700 : 400 }}>{row.landlord}</div>
                <div style={{ textAlign: 'center', fontSize: 12, color: row.pro.includes('Included') ? GREEN : row.pro === '—' ? '#1e3a5f' : '#94a3b8', fontWeight: row.pro.includes('Included') ? 700 : 400 }}>{row.pro}</div>
                <div style={{ textAlign: 'center', fontSize: 12, color: row.ent.includes('Included') || row.ent === 'Unlimited' || row.ent === 'White-label' || row.ent === 'Dedicated' ? GREEN : '#94a3b8', fontWeight: 600 }}>{row.ent}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 64 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 16 }}>
            {[
              ...
              { q: 'How does the Landlord add-on pricing work?', a: 'Start at $89/mo and add features individually. Each add-on activates instantly after payment through Stripe.' },
              { q: 'What happens if I exceed my unit limit?', a: 'You will be prompted to upgrade to the next plan. We never cut off your access — we give you 7 days to upgrade.' },
              ...
              { q: 'When will QuickBooks be available?', a: 'QuickBooks sync is coming in Q3 2026. Enterprise customers will get early access. All customers will be notified when it launches.' },
            ].map((faq, i) => (
        ...
        <div style={{ textAlign: 'center', padding: '48px 32px', background: 'linear-gradient(135deg,rgba(29,78,216,0.12),rgba(168,85,247,0.08))', border: '1px solid rgba(79,142,247,0.15)', borderRadius: 24 }}>
          ...
          <p style={{ fontSize: 15, color: '#64748b', marginBottom: 28, maxWidth: 440, margin: '0 auto 28px' }}>
            Join property managers and landlords across Oklahoma who use PropFlow OS to run their properties better.
          </p>
