...
const useTick = (base: number, v: number, ms = 3200) => {
...
const Dot = ({ color, sz = 7 }: { color: string; sz?: number }) => (
...
const Kpi = ({ label, value, unit, sub, color, icon, delay=0, crit=false }: any) => (
...
const Gauge = ({ label, temp, min, max, lo, hi, unit="°F" }: any) => {
...
const mkSpark = (b: number, n=14) => Array.from({length:n},(_,i) => +(b+Math.sin(i*.9)*3+(Math.random()-.5)*2).toFixed(1));
...
export default function MedFlowDashboard() {
  ...
  return (
    <>
      <GlobalStyles />
      <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:C.bg, color:C.txt, fontFamily:C.D, overflow:"hidden" }}>
        ...
        <header style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"13px 24px", borderBottom:`1px solid ${C.border}`, background:C.panel, flexShrink:0, gap:16 }}>
          <div style={{ minWidth:0 }}>
            <h1 style={{ fontSize:20, fontWeight:800, color:C.white, fontFamily:C.D, letterSpacing:-.3, whiteSpace:"nowrap", margin:0 }}>MedFlow OS — Command Center</h1>
            <p style={{ fontSize:10.5, color:C.dim, fontFamily:C.M, letterSpacing:.5, marginTop:1, marginBottom:0 }}>{FACILITIES[fac]} · Live overview</p>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"minmax(0,1fr) 320px", gap:16 }}>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:13, overflow:"hidden", minWidth:0 }}>
              ...
              <div style={{ display:"flex", gap:8, padding:"7px 18px", borderBottom:`1px solid ${C.border}` }}>
                {["Level","Message","Location","Time",""].map((h,i)=>(
                  <span key={h+i} style={{ ...(h==="Message"?{flex:1,minWidth:0}:h===""?{width:60,flexShrink:0}:{width:h==="Level"?72:h==="Location"?110:55,flexShrink:0}), fontSize:8.5, color:C.dim, letterSpacing:1.8, fontFamily:C.M, textTransform:"uppercase" }}>{h}</span>
                ))}
              </div>
              {ALERTS.map((a,i)=>(
        ...
        {modal && (
          <div onClick={()=>setModal(null)} style={{ position:"fixed", inset:0, background:"rgba(4,10,18,.88)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, backdropFilter:"blur(5px)" }}>
            <div onClick={(e:any)=>e.stopPropagation()} style={{ background:C.panel, border:`1px solid ${C.borderH}`, borderRadius:18, padding:"28px 30px", maxWidth:450, width:"92%", boxShadow:"0 30px 80px rgba(0,0,0,.65)" }}>
