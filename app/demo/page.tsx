...
const DEMOS = [
  { id: 'boxflow',  name: 'BoxFlow OS',   color: '#2563EB', grad: '#1d4ed8', desc: 'See fleet, dispatch, orders, production, and HR unified in one platform. Built for logistics, manufacturing, and distribution.' },
  { id: 'medflow',  name: 'MedFlow OS',   color: '#14D2C2', grad: '#0A6E68', desc: 'See live temperature monitoring, cold chain tracking, drug inventory, compliance, and compounding — all in one healthcare platform.' },
  { id: 'propflow', name: 'PropFlow OS',  color: '#4f8ef7', grad: '#3b6fd4', desc: 'See unit management, tenant tracking, GPS staff tracker, maintenance dispatch, and finance — all in one property management platform.' },
  { id: 'classflow', name: 'ClassFlow AI', color: '#a855f7', grad: '#7c3aed', desc: 'See AI-powered lesson creation, student management, adaptive learning pathways, and multi-language support — built for educators.' },
]
  ...
  async function handleSubmit() {
    ...
    try {
      await supabase.from('contact_leads').insert([{
        ...
        interest: `${DEMOS.find(d => d.id === selected)?.name} Demo`,
      }])
      setStatus('sent')
    } catch { setStatus('error') }
  }

  const inp = { width: '100%', background: 'rgba(7,15,36,0.8)', border: '1px solid rgba(37,99,235,0.2)', borderRadius: 10, padding: '13px 16px', fontSize: 15, color: '#f0f6ff', fontFamily: 'system-ui', outline: 'none', boxSizing: 'border-box' as const }
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#020818 0%,#070f24 100%)', color: '#f0f6ff', fontFamily: 'system-ui,sans-serif' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '80px 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          ...
          <h1 style={{ fontSize: 'clamp(32px,5vw,52px)', fontWeight: 900, lineHeight: 1.1, marginBottom: 16 }}>See It Running Inside a <span style={{ color: '#14D2C2' }}>Business Like Yours.</span></h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(380px,1fr))', gap: 24 }}>
          <div>
            <h2 style={{ fontSize: 16, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', letterSpacing: 2, marginBottom: 16 }}>Choose Your Platform</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 32 }}>
              {DEMOS.map(d => (
                <div key={d.id} onClick={() => setSelected(d.id)}
                  style={{ background: selected === d.id ? 'rgba(12,26,56,0.9)' : 'rgba(7,15,36,0.6)', border: `2px solid ${selected === d.id ? d.color : 'rgba(255,255,255,0.08)'}`, borderRadius: 16, padding: 20, cursor: 'pointer', transition: 'all 0.15s' }}>
          ...
          <div style={{ background: 'rgba(12,26,56,0.9)', border: `1px solid ${selectedDemo.color}30`, borderRadius: 24, padding: 36 }}>
            {status === 'sent' ? (
              <div style={{ textAlign: 'center', padding: '40px 0' }}>
                <div style={{ fontSize: 48, marginBottom: 20, fontWeight: 900, color: selectedDemo.color }}>+</div>
                <h2 style={{ fontSize: 24, fontWeight: 900, marginBottom: 12 }}>You are Booked!</h2>
                <p style={{ color: '#94a3b8', fontSize: 15, lineHeight: 1.6 }}>We will reach out within 24 hours to confirm your demo time. Check your inbox!</p>
              </div>
            ) : (
              <>
                <h2 style={{ fontSize: 20, fontWeight: 900, marginBottom: 6 }}>Request Your {selectedDemo.name} Demo</h2>
                <p style={{ color: '#64748b', fontSize: 13, marginBottom: 24 }}>We will reach out within 24 hours to schedule.</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div><label style={lbl}>Tell us about your operation</label><textarea style={{ ...inp, minHeight: 90, resize: 'vertical' as const }} placeholder="Fleet size, units managed, current software..." value={form.message} onChange={e => update('message', e.target.value)} /></div>
                </div>
                <button onClick={handleSubmit} disabled={status === 'sending' || !form.name || !form.email}
                  style={{ width: '100%', marginTop: 20, padding: '16px', background: `linear-gradient(135deg,${selectedDemo.grad},${selectedDemo.color})`, color: '#fff', border: 'none', borderRadius: 12, fontSize: 16, fontWeight: 800, cursor: 'pointer', fontFamily: 'system-ui', opacity: !form.name || !form.email ? 0.6 : 1 }}>
